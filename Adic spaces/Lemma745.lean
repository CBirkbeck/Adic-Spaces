...
**Sorry status:** This theorem is sorry'd. The difficulty is producing a rank-1
(MulArchimedean) valuation subring directly. Mathlib's domination theorem
(`Ideal.image_subset_nonunits_valuationSubring`) produces a valuation subring
satisfying conditions (1) and (2) but with an arbitrary-rank value group.

**Approaches considered and why they fail with current Mathlib:**

  (a) **`ValuationSubring.ofPrime` at a height-1 prime.** Given `V₀` from domination,
      `V₀.ofPrime Q₁` (Q₁ = height-1 prime) has MulArchimedean value group by
      `mulArchimedean_ofPrime_of_height_one`. However, condition (2) requires ALL
      I-images to lie in Q₁, which fails when some I-image has unit value in the
      maximal proper convex subgroup H₁ (a Z x Z counterexample shows the minimal
      prime over a principal ideal can have height > 1; see `ValuationPrimeConvex.lean`).

  (b) **Coarsening by `maxAvoid`.** Given `V₀` and an I-generator `j₀ ∉ 𝔭`,
      coarsening `V₀.valuation` by `maxAvoid(u₀⁻¹)` (u₀ = unit of V₀.valuation(j₀))
      gives condition (2) (since all I-unit values ≤ u₀ and u₀ ∉ maxAvoid) but the
      quotient `V₀.ValueGroupˣ / maxAvoid(u₀⁻¹)` is NOT MulArchimedean in general:
      elements outside `convexGenerated(u₀⁻¹) ⊃ maxAvoid(u₀⁻¹)` project to elements
      unbounded by powers of `[u₀⁻¹]`.

  (c) **Coarsening by `convexGenerated`.** Quotienting by `convexGenerated(u₀⁻¹)` makes
      u₀⁻¹ trivial in the quotient, so ALL I-generators (with unit values between u₀
      and 1) become units of the coarsened valuation subring, violating condition (2).

**Two correct approaches (neither yet formalized):**

  (i)  **Rank-1 domination (Bourbaki, Comm. Alg., Ch. VI, §8, No. 6):**
       Among all valuation subrings of `Frac(A/𝔭)` dominating the local subring
       generated by `A₀/𝔭₀` with I-images as nonunits, a maximal one with
       MulArchimedean value group exists by a Zorn argument that additionally
       controls the convex subgroup structure. This requires formalizing the
       rank-1 case of Zorn's lemma for local subrings.

  (ii) **Chevalley extension (Wedhorn, Lemma 7.44(3)):** Extend the I-adic
       valuation on `A₀` to `A` using I-adic completeness. The resulting
       valuation is automatically continuous and rank-1. This requires formalizing
       the I-adic valuation as a Mathlib `Valuation` and extending it to the
       fraction field of `A/𝔭`.
...
theorem exists_mulArchimedean_valuationSubring_of_prime
  ...
  -- SORRY: Requires rank-1 domination (Bourbaki, Comm. Alg., Ch. VI, §8, No. 6)
  -- or Chevalley extension (Wedhorn, Lemma 7.44(3)).
  --
  -- Mathlib's `Ideal.image_subset_nonunits_valuationSubring` gives V₀ with conditions
  -- (1) and (2) but arbitrary-rank value group. No post-hoc coarsening of V₀ can
  -- simultaneously achieve MulArchimedean and preserve condition (2):
  --   * Coarsening by maxAvoid(u₀⁻¹) preserves (2) but the quotient is not MulArchimedean.
  --   * Coarsening by convexGenerated(u₀⁻¹) gives MulArchimedean quotient (trivially)
  --     but makes I-generators into units, violating (2).
  --   * ofPrime at a height-1 prime gives MulArchimedean value group but condition (2)
  --     fails unless ALL I-images lie in the height-1 prime (not guaranteed).
  --
  -- A correct proof must construct V with all three properties simultaneously, either
  -- via a rank-1 Zorn argument or via the Chevalley extension. See docstring for details.
  sorry
...
which requires rank-1 domination (not yet available in Mathlib). See that theorem's
docstring for details and references.
