...
A previous attempt tried to coarsen this to rank 1 via `ValuationSubring.ofPrime`
at a minimal prime, but the required height-1 claim is false in general valuation
rings (see `ValuationPrimeConvex.lean` for counterexamples).
...
theorem exists_mulArchimedean_valuationSubring_of_prime
  ...
  -- Requires the rank-1 domination theorem: among all valuation subrings
  -- dominating a local subring, one with MulArchimedean value group exists.
  -- Mathlib currently only provides arbitrary-rank domination
