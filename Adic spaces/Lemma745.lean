...
/-- **Wedhorn Lemma 7.44 + Bourbaki CA VI.8.6 (algebraic core).**

In a valuation subring `V₀` of `K = Frac(A/𝔭)` dominating the pair of definition,
there exists a height-1 prime `Q` of `V₀` such that every I-generator's image
lands in `Q`.

**Construction:** The I-generators `s₁, ..., sₖ` are nonunits of `V₀`, so they lie
in the maximal ideal `𝔪`. Let `g = max{V₀.valuation(sᵢ)}` (the largest value among
generators). Since `g < 1` and `g ≠ 0`, the element `s_max` achieving `g` is a
nonzero non-unit. The principal ideal `(s_max)` contains all generators (in a
valuation ring, `v(sⱼ) ≤ v(s_max)` implies `s_max | sⱼ`). The radical `√(s_max)`
is the smallest prime `Q` containing `s_max`, and `Q` contains all generators.

**Height-1 property:** The prime `Q = √(s_max)` is height-1 in `V₀`. This follows
from the Bourbaki correspondence between primes of a valuation ring and convex
subgroups of its value group (Comm. Alg., Ch. VI, §4, No. 5): the convex subgroup
`H_Q` corresponding to `Q` is maximal proper (i.e., `convexSubgroupOfPrime V₀ Q`
is a maximal element in `{H : H ≠ ⊤}`), which is equivalent to `Q` having height 1.

**Status:** The existence of such a height-1 prime containing all I-generators
requires the Bourbaki correspondence (already formalized in `ValuationPrimeConvex.lean`)
plus the fact that the radical of a principal ideal generated by an element whose
value is outside every proper convex subgroup is height-1. The full formalization
of the latter fact (connecting `Ideal.radical` to `primeOfConvexSubgroup`) needs
~80 lines of additional infrastructure. -/
theorem exists_height_one_prime_containing_I
    {K : Type*} [Field K]
    (V₀ : ValuationSubring K)
    (f : Subring K) (hf : f ≤ V₀.toSubring)
    (I_image : Set K)
    (hI_nonunits : I_image ⊆ V₀.nonunits)
    (hI_ne : ∃ x ∈ I_image, x ≠ 0) :
    ∃ Q : Ideal V₀, Q.IsPrime ∧ Q ≠ ⊥ ∧
      (∀ (P : Ideal V₀) [P.IsPrime], P < Q → P = ⊥) ∧
      (∀ x ∈ I_image, ∀ (hx : x ∈ V₀.toSubring), (⟨x, hx⟩ : V₀) ∈ Q) := by
  -- This requires:
  -- 1. Construction of the radical of the ideal generated by I-images in V₀
  -- 2. Showing this radical is a height-1 prime (via value group analysis)
  -- 3. The Bourbaki correspondence connecting principal ideal radicals
  --    to convex subgroups generated by the value
  --
  -- The mathematical argument: in a valuation ring, all ideals are totally ordered.
  -- The I-images are nonunits, so they generate a proper ideal J ⊆ 𝔪 (maximal ideal).
  -- Since J is nonzero (from hI_ne), it has a minimal prime Q (exists by Zorn/well-ordering
  -- on the totally ordered set of primes). Q is height-1 because: if P < Q with P prime,
  -- then J is not contained in P (by minimality). But in a valuation ring with totally
  -- ordered ideals, J ⊄ P and P prime means P ⊂ J, so P ⊂ J ⊂ Q. The key step is
  -- showing P = ⊥, which follows from the Bourbaki correspondence: the convex subgroup
  -- of V₀.ValueGroupˣ corresponding to Q via primeOfConvexSubgroup/convexSubgroupOfPrime
  -- is maximal, so the one corresponding to P must be ⊤, forcing P = ⊥.
  sorry

/-- In a valuation subring, if `x` is in a prime `Q` and `x ∈ V₀`, then `x` is a
nonunit of `V₀.ofPrime Q`. -/
theorem ValuationSubring.mem_ofPrime_nonunits_of_mem_prime
    {K : Type*} [Field K]
    (V₀ : ValuationSubring K) (Q : Ideal V₀) [Q.IsPrime]
    {x : K} (hx : x ∈ V₀.toSubring) (hxQ : (⟨x, hx⟩ : V₀) ∈ Q) :
    x ∈ (V₀.ofPrime Q).nonunits := by
  rw [ValuationSubring.mem_nonunits_iff_exists_mem_maximalIdeal]
  refine ⟨V₀.le_ofPrime Q hx, ?_⟩
  rw [show (⟨x, V₀.le_ofPrime Q hx⟩ : V₀.ofPrime Q) =
    Subring.inclusion (V₀.le_ofPrime Q) ⟨x, hx⟩ from rfl]
  change Subring.inclusion (V₀.le_ofPrime Q) ⟨x, hx⟩ ∈
    IsLocalRing.maximalIdeal (V₀.ofPrime Q)
  have : ⟨x, hx⟩ ∈ V₀.idealOfLE (V₀.ofPrime Q) (V₀.le_ofPrime Q) := by
    rw [ValuationSubring.idealOfLE_ofPrime]; exact hxQ
  exact this
...
Huber ring, there exists a valuation subring `V` of `Frac(A/𝔭)` satisfying:
...
**Proof structure:** Get `V₀` from the domination theorem (`exists_valuationSubring_of_prime`,
conditions 1-2). Find a height-1 prime `Q` of `V₀` containing all I-images
(`exists_height_one_prime_containing_I`). Set `V = V₀.ofPrime Q`. Then:
- Condition 1: `range ≤ V₀ ≤ V₀.ofPrime Q = V` (by `le_ofPrime`)
- Condition 2: I-images are in `Q` hence in `(V₀.ofPrime Q).nonunits` (by `idealOfLE_ofPrime`)
- Condition 3: `mulArchimedean_ofPrime_of_height_one` gives MulArchimedean from height-1
...
theorem exists_mulArchimedean_valuationSubring_of_prime
  ...
  haveI : IsDomain (A ⧸ 𝔭) := Ideal.Quotient.isDomain 𝔭
  -- Step 1: Get V₀ from the domination theorem (conditions 1-2)
  obtain ⟨V₀, hrange₀, hnonunits₀⟩ := P.exists_valuationSubring_of_prime (𝔭 := 𝔭)
  -- Step 2: Identify the I-image set and its properties
  set φ := P.toFractionQuotient 𝔭 with hφ_def
  set I_image := φ.range.subtype '' (Ideal.map φ.rangeRestrict P.I : Set _) with hI_def
  -- I-images are nonunits of V₀
  have hI_nonunits : I_image ⊆ V₀.nonunits := hnonunits₀
  -- There exists a nonzero element in the I-image (from non-openness of 𝔭)
  have hI_ne : ∃ x ∈ I_image, x ≠ 0 := by
    obtain ⟨a₀, ha₀_I, ha₀_notp⟩ := P.exists_mem_I_not_mem_of_not_isOpen h𝔭
    refine ⟨φ a₀, Set.mem_image_of_mem _ (Ideal.mem_map_of_mem _ ha₀_I), ?_⟩
    intro h0
    apply ha₀_notp
    have : (algebraMap (A ⧸ 𝔭) (FractionRing (A ⧸ 𝔭)))
        ((Ideal.Quotient.mk 𝔭) (P.A₀.subtype a₀)) = 0 := h0
    rw [map_eq_zero_iff _ (IsFractionRing.injective (A ⧸ 𝔭) (FractionRing (A ⧸ 𝔭)))] at this
    exact Ideal.Quotient.eq_zero_iff_mem.mp this
  -- Step 3: Find a height-1 prime Q of V₀ containing all I-images
  obtain ⟨Q, hQ_prime, hQ_ne_bot, hQ_ht1, hQ_contains⟩ :=
    exists_height_one_prime_containing_I V₀ φ.range hrange₀ I_image hI_nonunits hI_ne
  haveI := hQ_prime
  -- Step 4: Set V = V₀.ofPrime Q
  refine ⟨V₀.ofPrime Q, ?_, ?_, ?_⟩
  -- Condition 1: Range containment (V₀ ≤ V₀.ofPrime Q, so range ≤ V₀ ≤ V)
  · exact hrange₀.trans (V₀.le_ofPrime Q)
  -- Condition 2: I-images land in nonunits of V₀.ofPrime Q
  · intro y hy
    -- y ∈ I_image, so y ∈ V₀.nonunits (hence y ∈ V₀ and y is in maximal ideal)
    have hy_V₀ : y ∈ V₀.nonunits := hI_nonunits hy
    have hy_mem : y ∈ V₀.toSubring :=
      ValuationSubring.nonunits_subset hy_V₀
    -- y is in Q (by hQ_contains)
    have hyQ : (⟨y, hy_mem⟩ : V₀) ∈ Q := hQ_contains y hy hy_mem
    -- Therefore y is a nonunit of V₀.ofPrime Q
    exact ValuationSubring.mem_ofPrime_nonunits_of_mem_prime V₀ Q hy_mem hyQ
  -- Condition 3: MulArchimedean value group (from height-1 prime)
  · exact ValuationSubring.mulArchimedean_ofPrime_of_height_one V₀ Q hQ_ne_bot hQ_ht1
