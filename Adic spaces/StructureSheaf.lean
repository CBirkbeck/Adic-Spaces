...
theorem isSheafy_ofStronglyNoetherianTate_flat
  ...
  isEmbedding_productRestriction C := by
    ...
    · -- Inducing: the product restriction reflects the topology.
      ...
      --
      -- Proof route (Wedhorn Remark 8.20, Corollary 8.31):
      -- (a) By Prop 8.15, B := presheafValue C.base is a strongly noetherian Tate ring
      --     and each restrictionMap B → presheafValue D is a rational localization.
      --     [MISSING: full Prop 8.15 identification.]
      -- (b) Each restriction is a flat localization → continuous and open
      --     (topologically of finite type, Wedhorn Prop 6.29).
      --     [AVAILABLE modulo (a): localization maps are open for adic topologies.]
      -- (c) Product of localizations at a faithful cover is faithfully flat
      --     (Wedhorn Cor 8.31, Mathlib: Module.FaithfullyFlat.of_comap_surjective).
      --     [AVAILABLE modulo (a).]
      -- (d) Faithfully flat + injective product map → topological inducing:
      --     The product map B → ∏ B[1/D.s] is injective (separation, proved)
      --     and the product topology on ∏ B[1/D.s] pulls back to the adic
      --     topology on B. This is because:
      --     - B → ∏ B[1/D.s] is a closed map (faithfully flat + noetherian),
      --     - The preimage topology equals the original adic topology
      --       (open sets in B are exactly preimages of opens in the product).
      --     Alternatively: use that the I-adic topology on B is generated by
      --     powers of the ideal of definition, and each localization preserves
      --     this filtration (Prop 8.15 identification).
      --
      -- Key Mathlib tools:
      -- - Topology.IsInducing.mk (induced topology characterization)
      -- - Module.FaithfullyFlat (faithfully flat descent)
      -- - IsLocalization.Away (localization at element)
      sorry
