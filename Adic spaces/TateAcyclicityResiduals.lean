...
is proved by composing four substantive open lemmas (W1–W4) plus a
closed graft-preservation helper (W5). The architecture follows
**Wedhorn's two-stage proof** (round-6 reviewer guidance, ChatGPT Pro):
standard-cover input → first-stage Laurent cover (Cor 7.32) → per-leaf
unit-ratio refinement at the *relative* `𝒪(L)` level → transport to
absolute → graft → **prune** (the prune cleans up `allNodesDisjoint`
without needing a cross-leaf hypothesis; this is round-6 Option C).
...
* **W2** `exists_first_stage_laurent_tree_unit_generated` — Wedhorn
  ...
  Laurent tree on `s⁻¹·S` with `allSplitsInducing` and, at each leaf
  `L`, the **restricted standard cover `U|L` is generated by units**
  in `𝒪_X(L)` (Wedhorn's actual Step (ii) output).
* **W3** `unitGeneratedCover_has_relative_ratioLaurentRefinement` —
  Wedhorn 8.34 *Step (iii)*: a rational cover generated by units has
  a Laurent-ratio refinement, stated **at the relative `𝒪(L)` level**
  (cleaner Wedhorn alignment per round-6 reviewer; the ratios live
  in `𝒪(L)`, not in `A`).
* **W3-transport** `relative_laurent_tree_to_absolute` — transport
  theorem from `LaurentTree (presheafValue L)` to `LaurentTree A`,
  preserving the three predicates via `presheafValue_relative_equiv`
  and its homeomorphism (Group III).
* **W4** `prune_preserves_refinement_and_inducing` — Wedhorn 8.34
  *graft cleanup* (round-6 Option C): any Laurent tree with `Refines`
  and `allSplitsInducing` has a pruned equivalent with same
  `toCovering`, `Refines`, `allSplitsInducing`, **and**
  `allNodesDisjoint`. Replaces the round-6 W4 (cross-leaf
  disjointness) which was mathematically false for abstract
  `inner_of`.

**Closed:**
* **W5** `graftAt_allNodesDisjoint` — internal helper for the prune;
  preserved as a useful structural-induction lemma for downstream code.

The proof of I.1 glues W1–W4 via the existing graft preservation
lemmas (`LaurentTree.Refines_graftAt`,
`LaurentTree.allSplitsInducing_graftAt`) + W4's prune. -/
...
/-- **Restricted standard cover generated by units (round-6 def).**
For a first-stage Laurent leaf `L`, the restricted cover `U|L`
(= the standard cover by `S` restricted to `L`) is *generated by
units* in `𝒪_X(L)` if there is a sub-family `I_units ⊆ S` such that
(a) each `f ∈ I_units` gives `s⁻¹·f` a unit at `L`, and (b) the
`I_units`' plus-pieces (at `C.base` level) cover `L`'s rational open.

This is the precise quantifier structure of Wedhorn 8.34 Step (ii):
*for every first-stage leaf `L`, the restricted cover `U|L` is
generated by units*. Both clauses (a) and (b) are required — the
unit property alone is vacuous if `I_units = ∅`, and the
cover-decomposing alone doesn't guarantee the f's are units. -/
...
theorem unitGeneratedCover_has_relative_ratioLaurentRefinement
    ...
    -- The relative inner tree has `allSplitsInducing`. The descent
    -- to C-pieces is handled by W3-transport below, which produces
    -- the absolute tree carrying `Refines L C`. The prune step (W4)
    -- cleans up `allNodesDisjoint`.
...
theorem relative_laurent_tree_to_absolute
    ...
    -- Typeclass hypotheses on `presheafValue L` (as in W3):
    [IsTopologicalRing (presheafValue L)] [PlusSubring (presheafValue L)]
    [IsHuberRing (presheafValue L)] [HasLocLiftPowerBounded (presheafValue L)]
    (L_rel : RationalLocData (presheafValue L))
    (inner_rel : LaurentTree (presheafValue L))
    (_h_split_rel : inner_rel.allSplitsInducing L_rel) :
    -- The transport produces an absolute tree with both
    -- `allSplitsInducing L` AND `Refines L C`. The Refines comes
    -- from descending each relative leaf to A via III.1 + clearing
    -- denominators, then composing with `refines_contain` to get
    -- containment in some C-piece.
    ∃ inner_abs : LaurentTree A,
      inner_abs.allSplitsInducing L ∧
      inner_abs.Refines L C := by
  sorry

/-- **(W4) Prune preserves refinement and inducing (Option C).**
Any Laurent tree with `Refines` and `allSplitsInducing` at a base
has a *pruned equivalent* with the same `toCovering`, the same
`Refines` and `allSplitsInducing`, and additionally
`allNodesDisjoint`. The prune deduplicates leaves and collapses
trivial splits without changing the represented cover.

**Round-6 revision (ChatGPT Pro): replaces the false cross-leaf
disjointness claim.** The earlier W4 (cross-leaf disjointness for
abstract `inner_of`) was mathematically false. The reviewer
suggested three resolutions; we chose Option C (prune theorem) as
the most general: it handles disjointness as a Lean bookkeeping
artifact via a pruning operation, decoupled from the canonical
construction's specific structure.

**Lean encoding.** The prune is an existential — it asserts the
existence of a pruned tree without specifying its construction.
Downstream consumers (I.1's proof) only need the existential.
A concrete `prune` function could be defined for performance, but
the existential is sufficient for the IsSheafy proof. -/
