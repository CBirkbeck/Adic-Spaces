...
theorem unitGeneratedCover_has_relative_ratioLaurentRefinement
    ...
    -- The base `L_rel : RationalLocData (presheafValue L)` is the
    -- relative locality datum at `L` (identity-like).
    (L_rel : RationalLocData (presheafValue L))
    -- The *relative unit-generated cover* over `O(L)` that
    -- `inner_rel` must refine. In Wedhorn's terms: the cover
    -- generated by the units `image_L(s⁻¹·f)` for `f ∈ I_units`.
    -- We take it as a parameter (W3 doesn't construct it; the W2
    -- output carries the I_units that determine it). The
    -- relationship between `unitCover` and `I_units` is captured
    -- by `_h_unit_generated`'s clause (d).
    (unitCover : RationalCovering (presheafValue L))
    (_h_unitCover_from_I_units : unitCover.base = L_rel) :
    -- Round-8 strengthened output: the relative tree REFINES the
    -- relative unit-generated cover (Wedhorn Step (iii) literally),
    -- not just `allSplitsInducing`. This is the data needed to
    -- give the absolute `Refines L C` claim via W3-transport.
...
theorem relative_laurent_tree_to_absolute
    ...
    [IsHuberRing (presheafValue L)] [HasLocLiftPowerBounded (presheafValue L)]
    (L_rel : RationalLocData (presheafValue L))
    -- The relative unit-generated cover (from W3's input):
    (unitCover : RationalCovering (presheafValue L))
    (_h_unitCover_from_I_units : unitCover.base = L_rel)
    -- The relative inner tree REFINING unitCover (strengthened W3
    -- output per round-8 reviewer):
    (inner_rel : LaurentTree (presheafValue L))
    (_h_refines_rel : inner_rel.Refines L_rel unitCover)
    (_h_split_rel : inner_rel.allSplitsInducing L_rel) :
    -- Round-8 transport chain (per reviewer):
    --   inner_rel refines unit-generated cover over O(L)
    --   ↓ unit-generated cover over O(L) refines restricted standard
    --     cover over L (via _h_unit_generated clauses (c) + (d))
    --   ↓ restricted standard cover over L refines C
    --     (via _hS_contain : refines_contain C S, project-side)
