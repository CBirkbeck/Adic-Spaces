...
theorem exists_first_stage_laurent_cover
    ...
    ∃ (s : Aˣ) (V : LaurentTree A),
      ...
      -- At every leaf L of V, the σ-minus indices of S (= those f for
      -- which s⁻¹·f is a unit at L's presheaf value) form a *sub-family*
      -- of S whose plus-pieces at C.base.s cover the leaf L's rational
      -- open. This is the precise "U|L is generated by units in 𝒪_X(L)"
      -- content of Wedhorn pp. 83–84: at every leaf, the restriction of
      -- the standard cover is determined by the unit family at L.
      ∀ L ∈ V.leaves C.base,
        ∃ I_units : Finset A,
          I_units ⊆ S ∧
          (∀ f ∈ I_units,
            IsUnit (L.canonicalMap (((s⁻¹ : Aˣ) : A) * f))) ∧
          -- I_units's plus-pieces cover L (as a sub-cover of U|L):
