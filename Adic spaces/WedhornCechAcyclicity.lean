...
/-- **Cor 7.32 hypothesis-supply lemma 1**: a pair-of-definition `P` whose
`A₀` is contained in `A⁺` (the hypothesis required by Cor 7.32 / the
project's `exists_dominating_unit`).

For Wedhorn 8.34's application, the right choice of pair is the one
attached to the base of the rational covering being analyzed (project's
`C.base.P`), assuming `C.base.P.A₀ ≤ A⁺` (a standing assumption for
project-level RationalLocData). -/
theorem exists_pair_with_A₀_subset_Aplus
  ...
  -- Wedhorn's setting: every Tate ring has a pair `(A₀, I)` with
  -- `A₀ ⊆ A⁺`. Concrete construction in the project might use a
  -- "smallest A₀" containing topnilp elements.
  sorry

/-- **Cor 7.32 hypothesis-supply lemma 2**: every Tate-ring pair `P` admits a
generator `π : P.A₀` of `P.I` that is a topologically nilpotent unit in `A`. -/
theorem exists_pseudouniformizer_of_tate
  ...
  -- For Tate rings, the topologically-nilpotent units generate the
  -- topology; some such unit lies in P.I and generates it.
  sorry
...
theorem cor_7_32_dominating_unit
  ...
  -- Compose the hypothesis-supply lemmas with `exists_dominating_unit`.
  obtain ⟨P, hP⟩ := exists_pair_with_A₀_subset_Aplus (A := A)
  obtain ⟨π, hI, hπ_tn, hπ_unit⟩ := exists_pseudouniformizer_of_tate P
...
Composed from sub-lemmas (a) + (b) + (c) above. -/
theorem wedhorn_lemma_834 [DecidableEq A]
  ...
  -- Pick V|U for each U: we use the project's V_restr-construction.
  -- For now, project this work into a sub-lemma to keep the composition
  -- visible. (Each lemma is a Wedhorn-faithful obligation.)
  choose C_at_Vj _hC_at_base hC_at_unit hC_at_pieces _hC_at_cover using
    fun Vj (hVj : Vj ∈ V.covers) => hV_unit_restrictions Vj hVj
  -- Bridge: assemble the project-form Prop A.3(1) with these families.
  -- The V_restr_at family is constructed structurally (project-side):
  -- restrict V's Laurent generators to each U ∈ C.covers.
  -- The C_restr_at family comes from `hV_unit_restrictions`.
  sorry
...
/-- **Project-side bridge 1**: a `RationalCovering A` corresponds to an
abstract `FiniteCover ↥(Spa A A⁺) ι` for its index type ι. -/
def RationalCovering.toFiniteCover [IsHuberRing A] (C : RationalCovering A) :
    FiniteCover ↥(Spa A A⁺) C.covers := by
  -- Build a FiniteCover: pieces are Subtype.val ⁻¹' (rationalOpen D.T D.s)
  -- for D ∈ C.covers (= ↥C.covers indexed pieces).
  -- The cover-of-Spa-by-C.base relation needs to be set up; here we cover
  -- C.base's rational open by its pieces.
  sorry
...
def RationalCovering.toRefinement [IsHuberRing A]
    ...
    (_h_refines : ∀ D' ∈ C'.covers, ∃ D ∈ C.covers,
        rationalOpen D'.T D'.s ⊆ rationalOpen D.T D.s) :
    Refinement C'.toFiniteCover C.toFiniteCover := by
  -- Construct: the index map κ → ι sends each C'-piece D' to a C-piece D
  -- containing it; the subset proof comes from h_refines.
  sorry
...
theorem double_restriction_acyclicity [DecidableEq A]
    ...
    (h_E_pieces : ∀ E' ∈ E.covers,
      ∃ D' ∈ C'.covers,
        rationalOpen E'.T E'.s ⊆ rationalOpen D'.T D'.s) :
    E.IsOXAcyclic := by
  -- Step a: E inherits IsGeneratedBy T from C'.
  have h_E_gen : E.IsGeneratedBy T :=
    restricted_cover_inherits_IsGeneratedBy C' T h_C'_gen D E h_E_base h_E_pieces
  -- Step b: apply Lemma 8.34 to E with T.
  exact wedhorn_lemma_834 E T h_E_gen
