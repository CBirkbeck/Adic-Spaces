...
theorem wedhorn_lemma_834_propA3_part1_gluing
  ...
  have hV'_sub_D : rationalOpen V'.T V'.s ⊆ rationalOpen D.1.T D.1.s := by
    ...
    exact this
  -- Use the parametric inner_identity_generic for the per-V' identity.
  have h_inner_identity :
      yV ⟨V', hV'_in_V⟩ = restrictionMap D.1 V' hV'_sub_D (f D) :=
    inner_identity_generic ⟨V', hV'_in_V⟩ D hV'_sub_D
