...
`E := C' restricted to D ∈ C.covers`, used to unblock Prop A.3(2) gluing. -/
noncomputable def RationalCovering.restrictToPiece
    (C' : RationalCovering A) (h_refines : ∀ D' ∈ C'.covers, ∃ D ∈ C'.covers,
        rationalOpen D'.T D'.s ⊆ rationalOpen D.T D.s)
    (D : RationalLocData A) (_hD_covers : ∀ v ∈ rationalOpen D.T D.s,
      ∃ D' ∈ C'.covers, v ∈ rationalOpen D'.T D'.s ∧
        rationalOpen D'.T D'.s ⊆ rationalOpen D.T D.s) :
    RationalCovering A := by
  -- Build C' restricted to D: pieces are C'-pieces refining into D.
  sorry
