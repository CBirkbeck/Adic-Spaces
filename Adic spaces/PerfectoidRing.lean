...
class IsPerfectoidRing (p : ℕ) [Fact (Nat.Prime p)]
  ...
  /-- The ring is `p`-torsion-free: if `p · a = 0` then `a = 0`.
  This holds for all perfectoid rings (Scholze-Weinstein, *Berkeley Lectures*, Lemma 6.2.1).
  It follows from the perfectoid conditions: `p = c · ϖ^p` with `ϖ` topologically nilpotent
  implies `p` generates a non-degenerate ideal, and the T₀ separation axiom prevents
  nonzero `p`-torsion.
  (Scholze-Weinstein, *Berkeley Lectures on p-adic Geometry*, Lemma 6.2.1) -/
  p_regular : ∀ a : A, (p : A) * a = 0 → a = 0
  /-- In the tilt `A♭ = Perfection(A°/(p), p)`, the kernel of `Perfection.coeff 0`
  is "principally generated by any nonzero element": if `a, b ∈ ker(coeff 0)` and
  `a ≠ 0`, then `a | b`. This is a consequence of the tilt being a valuation ring
  (for perfectoid fields) or of the pseudo-uniformizer structure (in general).
  It is the key algebraic ingredient in proving that `ker(θ)` is principal
  (Berkeley Lectures, Lemma 6.2.8, division step).
  (Scholze-Weinstein, *Berkeley Lectures on p-adic Geometry*, Lemma 6.2.8) -/
  tilt_ker_coeff_dvd :
    ∀ [inst : Fact (¬ IsUnit (p : ↥(powerBoundedSubring.toSubring A)))],
    ∀ (a b : PreTilt (↥(powerBoundedSubring.toSubring A)) p),
    Perfection.coeff _ p 0 a = 0 → Perfection.coeff _ p 0 b = 0 → a ≠ 0 →
    ∃ q, b = a * q
