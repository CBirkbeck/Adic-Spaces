...
/-- Every finite subset of a topological ring is bounded (Remark 5.28(1) of Wedhorn). -/
...
This shows that `A°°` is radical: if `a ^ m ∈ A°°` then `a ∈ A°°`
(Proposition 5.30(4) of Wedhorn). -/
theorem IsTopologicallyNilpotent.of_pow [IsTopologicalRing A] {a : A} {m : ℕ} (hm : 0 < m)
  ...
  -- {a^0, a^1, ..., a^(m-1)} is bounded (finite set)
