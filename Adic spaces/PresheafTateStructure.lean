...
**Non-circular proof route** (via AdicCompletionBridge, ~105 lines of new infrastructure):

1. [DONE] `locSubring_subspace_eq_adic`: subspace uniformity on locSubring = J-adic.
   So `ι := coeRingHom ∘ subtype : locSubring → presheafValue` is a uniform inducing.
2. [~30 lines] `Completion.extension ι` is a uniform inducing (by `isUniformInducing_extension`)
   with image `closure(range ι) = ringOfDef`. This gives a homeomorphism
   `Completion(locSubring, J-adic) ≃ₜ ringOfDef`.
3. [DONE] `adicCompletionRingEquiv` (AdicCompletionBridge.lean) gives a homeomorphism
   `Completion(locSubring) ≃ₜ AdicCompletion(locIdeal, locSubring)`.
4. [~15 lines] `ker(evalₐ n)` is closed in `AdicCompletion` (preimage of `{0}` in
   discrete `locSubring/locIdeal^n` under continuous `evalₐ`).
5. [~40 lines] `ker(evalₐ n) = Ideal.map of (locIdeal^n)` via `AdicCompletion.map_exact`
   on `0 → locIdeal^n → locSubring → locSubring/locIdeal^n → 0` together with
   `ofTensorProduct_bijective_of_finite_of_isNoetherian` (showing the range of
   `map incl` = `AdicCompletion(J,R) * of(generators)` = `Ideal.map of (J^n)`).
6. [~20 lines] Transfer: under composed homeomorphism, `idealOfDef^n = Ideal.map g (J^n)`
   corresponds to `Ideal.map of (J^n) = ker(evalₐ n)`, which is closed. Then
   `IsClosed.isClosedMap_subtype_val` gives closedness in `presheafValue`.

**Why all simpler approaches are circular**: The sandwich
`coe '' locNhd n ⊆ val '' idealOfDef^n ⊆ closure(coe '' locNhd n)` gives
`val '' idealOfDef^n = closure(coe '' locNhd n) ∩ ringOfDef` IF the reverse
inclusion `closure_locNhd_sub_idealOfDef_pow` holds. But that theorem USES
this closedness result. Every approach via the sandwich is therefore circular.

**See also**: `AdicCompletionBridge.lean` for the bridge machinery. -/
private theorem idealOfDef_pow_val_isClosed (D₀ : RationalLocData A)
  ...
  -- NON-CIRCULAR PROOF OUTLINE (verified mathematically, awaits Lean infrastructure)
  --
  -- The proof has two parts:
  -- (A) idealOfDef^n is closed in the subspace topology on ringOfDef.
  -- (B) val '' (closed subset of closed subring) is closed (IsClosed.isClosedMap_subtype_val).
  --
  -- Part (B) follows immediately from hclosed_ring + IsClosed.isClosedMap_subtype_val.
  -- Part (A) is the substance of the proof:
  --
  -- STEP 1 [DONE]: locSubring_subspace_eq_adic (line ~159 above)
  --   The subspace uniformity on locSubring in Localization.Away s equals
  --   the locIdeal-adic uniformity. This makes the inclusion
  --   ι := coeRingHom ∘ subtype : locSubring → presheafValue
  --   a uniform inducing (from J-adic to completion uniformity).
  --
  -- STEP 2 [NEEDS ~30 lines]: ringOfDef is the completion of locSubring
  --   Since ι is a uniform inducing with T0 complete target (presheafValue):
  --   - Completion.extension ι : Completion(locSubring) → presheafValue is
  --     a uniform inducing (by isUniformInducing_extension).
  --   - range(Completion.extension ι) = closure(range ι) = ringOfDef.
  --   - Completion.extension ι restricts to a homeomorphism
  --     Completion(locSubring, J-adic) ≃ₜ ringOfDef.
  --
  -- STEP 3 [DONE]: AdicCompletionBridge (AdicCompletionBridge.lean)
  --   adicCompletionRingEquiv : Completion(locSubring) ≃+* AdicCompletion(J, locSubring)
  --   is a homeomorphism (the forward/backward maps are AbstractCompletion.compare,
  --   which are uniformly continuous with uniformly continuous inverses via compareEquiv).
  --
  -- STEP 4 [NEEDS ~15 lines]: ker(evalₐ J n) is closed in AdicCompletion
  --   evalₐ J n : AdicCompletion(J, locSubring) →ₐ locSubring/J^n is continuous
  --   (composition of Subtype.val : AdicCompletion → Pi and Pi.apply n, both
  --   continuous, postcomposed with the AlgEquiv from ideal_smul_top_eq_self).
  --   The target locSubring/J^n has discrete topology (J^n is open in J-adic).
  --   So {0} is closed, and ker(evalₐ n) = evalₐ⁻¹({0}) is closed.
  --
  -- STEP 5 [NEEDS ~40 lines]: ker(evalₐ J n) = Ideal.map (of J) (J^n) in AdicCompletion
  --   The ⊆ direction: ker(evalₐ n) = range(AdicCompletion.map (Submodule.subtype (J^n)))
  --   by AdicCompletion.map_exact applied to 0 → J^n → locSubring → locSubring/J^n → 0.
  --   The ⊇ direction: of(x) ∈ ker(evalₐ n) for x ∈ J^n (since evalₐ n (of x) = mk x = 0).
  --   Equality: range(map incl) is an AdicCompletion(J,R)-submodule generated by
  --   {of(a_i)} for generators a_i of J^n (by ofTensorProduct_bijective_of_finite_of_isNoetherian,
  --   giving AdicCompletion(J, J^n) ≃ AdicCompletion(J, R) ⊗_R J^n, so range(map incl) =
  --   AdicCompletion(J,R) · of(J^n) = Ideal.map of (J^n)).
  --
  -- STEP 6 [NEEDS ~20 lines]: Transfer through Steps 2+3
  --   Under the composed homeomorphism ringOfDef ≃ₜ Completion(locSubring) ≃ₜ AdicCompletion:
  --   - g(x) ∈ ringOfDef maps to coe(x) ∈ Completion maps to of(x) ∈ AdicCompletion
  --   - idealOfDef^n = Ideal.map g (J^n) maps to Ideal.map of (J^n) = ker(evalₐ n)
  --   - ker(evalₐ n) is closed (Step 4), so idealOfDef^n is closed in ringOfDef
  --
  -- TOTAL MISSING INFRASTRUCTURE: ~105 lines
  --   - Homeomorphism Completion(locSubring) ≃ₜ ringOfDef (Step 2)
  --   - Continuity of evalₐ for bridge topology (Step 4)
  --   - ker(evalₐ n) = Ideal.map of (J^n) via map_exact + tensor product (Step 5)
  --   - Diagram chase for Step 6
  sorry
