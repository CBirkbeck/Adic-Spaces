...
**Status (2026-04-16)**: open — pending Example 6.38 (Tate algebra
quotient identification of `presheafValue`).
...
**The needed lemma** (clean signature, no extra hypotheses on `A`):

```
theorem presheafValue_tateAlgebra_quotient_iso
    [IsTateRing A] [IsNoetherianRing A] [T2Space A] [NonarchimedeanRing A]
    (D : RationalLocData A) :
  presheafValue D ≃+* (TateAlgebra A on |D.T|+1 vars) ⧸ I_D
```

(Wedhorn Example 6.38, where `I_D` is the closed ideal generated by
`{1 - D.s · X₀, t·X₀ - Xᵢ : tᵢ ∈ D.T}`). This identification is
ALGEBRAIC (universal property of Tate-algebra completion), not just
topological, and intertwines `restrictionMapHom` with the natural map
between the Tate algebra quotients (which is induced by the inclusion
of generators). Injectivity of the latter is then a kernel computation
in finitely-generated noetherian Tate algebras (where ideals are closed
by Wedhorn Prop 6.17), reducible to Lemma 8.31 / Cor 8.32 at the
algebraic level.
...
**Signature analysis**: keeping the four typeclass arguments
`[IsTateRing A] [IsNoetherianRing A] [T2Space A] [NonarchimedeanRing A]`
is sufficient for the Example 6.38 identification (these are the same
hypotheses required to construct the Tate algebra and apply Prop 6.17
for closed ideals). No `[DiscreteTopology A]`, `[IsDomain A]`,
`MulArchimedean` or `hZavyalov` is needed if Example 6.38 is proved
in the standard Tate setup; in particular, the signature of
`restrictionMapHom_injective` does not need to change. -/
theorem restrictionMapHom_injective
  ...
  -- BLOCKED: see preceding doc-block.
  -- The cleanest route is via Example 6.38 (Tate-algebra quotient
  -- identification of `presheafValue`), which is not yet built.
  -- All naive routes (completion-extension uniform inducing,
  -- faithful flatness of single restriction map, module flatness over
  -- A, algebraic-kernel + density) are documented above as FALSE or
  -- INSUFFICIENT.
  sorry
