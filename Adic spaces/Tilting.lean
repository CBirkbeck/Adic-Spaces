  ...
  ker_theta_principal_aux (p : ℕ) [Fact (Nat.Prime p)]
    ...
    have hb : ∃ (xi : Ainf p A), xi ∈ RingHom.ker (PerfectoidRing.theta p A) ∧
      ...
      · refine ⟨0, Ideal.zero_mem _, fun x hx => ⟨0, ?_⟩⟩
        ...
        rw [this]; ring
      · -- Nontrivial kernel: Berkeley Lectures, Lemma 6.2.8 (pp.46-47).
        -- PROOF SKETCH (~80 lines of Lean, all prerequisite API proved):
        --
        -- (A) Construct ξ ∈ ker(θ) with ξ.coeff 0 ≠ 0:
        --   Find α ∈ ker(PreTilt.coeff 0 : tilt → A°/(p)), α ≠ 0. Since α.untilt ≡ 0
        --   mod p, write α.untilt = p*d. By theta_surjective, pick w with θ(w) = d.
        --   Set ξ = [α] - p*w. Then θ(ξ) = α.untilt - p*d = 0 (fontaineTheta_teichmuller)
        --   and ξ.coeff 0 = α ≠ 0.
        --
        -- (B) Division step: ∀ y ∈ ker(θ), ∃ q r, y = ξ*q + p*r ∧ r ∈ ker(θ):
        --   Write y = [y.coeff 0] + p*y' (eq_teichmuller_add_p_mul). Since θ(y) = 0,
        --   PreTilt.coeff 0 (y.coeff 0) = 0 (fontaineTheta_teichmuller + mk_untilt).
        --   So y.coeff 0 ∈ ker(PreTilt.coeff 0). The ideal coeff_0(ker(θ)) equals
        --   ker(PreTilt.coeff 0), and α = ξ.coeff 0 divides y.coeff 0 in the tilt
        --   (since α generates this kernel via the pseudo-uniformizer structure).
        --   Set q = [c₀] where y.coeff 0 = α*c₀, r = y' - ξ'*[c₀] where ξ = [α]+p*ξ'.
        --   Then p*θ(r) = 0 and θ(r) = 0 follows from p-adic Hausdorffness of A°.
        --
